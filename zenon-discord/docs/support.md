...
- `/지원패널`(admin·support) → 활성 서버의 `건의-문의-버그제보` 채널에 **지원 센터 패널** 게시. 버튼: [문의하기] → 티켓 생성, [FAQ 보기] → FAQ 패널(ephemeral), [버그제보 안내] → `/버그제보 대상 심각도` 안내.
...
- 🟢 **구현(2026-06-09, 2026-07-02 패널 추가): `modules/support/tickets.py` + db v10 `tickets` + `core/tickets.py`.** `/문의`(동시 1개 제한, @everyone 숨김+개설자/admin/support 가시) · `/지원패널`(지원 채널 고정 안내+버튼) · `/티켓종료`·[종료] 영구버튼(개설자/운영) · 종료=**잠금·아카이브**(채널 삭제 X, `[종료]` 프리픽스 + 개설자 접근 해제 — §5 삭제vs아카이브 = 아카이브 채택, 생애주기 정합). 생성 카테고리 = `CATEGORY_티켓_ID`(.env, 미설정 시 무카테고리). mod_log(ticket_open/close).
...
|---|---|
| `/버그제보` `/문의` `/faq` | 공통(유저) |
| `/지원패널` · `/티켓종료`(타인 티켓) · `/faq추가·수정·삭제` | `requires_permission`(admin·support) |
