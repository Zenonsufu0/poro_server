...
| 영역 | 보호 | 방식 | 비고 |
|---|---|---|---|
| **스폰 = 인증 대기 허브** | ✅ 보호 | spawn-protection + ZenonMonCore 인증 경계 | 미인증 Adventure 감금, 인증 후 야생 이동 |
...
- 보호가 필요한 것은 **인증 대기 허브**뿐 → 바닐라 `spawn-protection` + ZenonMonCore 인증 경계로 처리한다.
- 인증 완료 유저는 허브 밖 야생 지점으로 이동하며, 허브 반경 재진입 시 다시 야생으로 이동한다.
...
- **전제**: **월드 스폰 = 인증 대기 허브 중심**으로 설정 (`/setworldspawn` 또는 월드 스폰 고정). 허브가 스폰과 떨어져 있으면 spawn-protection으로는 못 막음 → 2차(아래) 필요.

## 2-1. ZenonMonCore 인증 경계

- `core.json → hub.spawn` = 인증 대기 허브 중심.
- `core.json → discordAuth.confineRadius` = 인증 대기 허브 반경.
- 미인증자는 Adventure 모드로 고정되고, 반경 밖/타 차원 이동 시 허브로 복귀한다.
- 인증 완료 시 Survival 모드로 전환되고, 야생 이동 기능이 활성화되어 있으면 `/zenonmon wild`와 같은 안전 지표 탐색 로직으로 야생에 배치된다.
- 인증 완료 유저가 허브 반경 안에 들어오면, 야생 이동 기능이 활성화되어 있는 경우 야생으로 다시 이동한다.
...
- **미인증 플레이어**: 인증 대기 허브 반경 내 Adventure 모드, 허브 이탈 불가.
- **인증 플레이어**: 야생 이동 기능이 활성화되어 있으면 허브 반경 재진입 불가, 그 외 일반 월드는 자유.
